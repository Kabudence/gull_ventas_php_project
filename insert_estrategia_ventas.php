...
if (empty($errors)) {
    ...
    if (empty($errors)) {
        ...
        if (empty($product["image_name"])) {
            ...
            echo $product_inserted_id = insertar_estrategia($product);
            echo "<br> des";
            ...
            if ($product_inserted_id > -1) {
                foreach ($product_images as $image) {
                    ...
                    if (insert_estrategia_ventas_images($images) > -1) {
...
if ($success) {
    ...
    $redirect_to = root_dir() . 'add_estrategia_ventas.php?errormsg=true&' . $parametros;
    header('Location: ' . $redirect_to);
}
echo "fin";
//}