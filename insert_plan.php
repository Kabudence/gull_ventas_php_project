...
if (empty($errors)) {
    ...
    if (empty($errors)) {
        ...
        if (empty($product["image_name"])) {
            ...
            echo $product_inserted_id = insertar_plan($product);
            echo "<br> des";
            ...
            if ($product_inserted_id > -1) {
                foreach ($product_images as $image) {
                    ...
                    if (insert_plan_images($images) > -1) {
                        $_SESSION['product_msg'] = "Plan añadido con éxito.";
                    }
                }
                echo "<br> des3";
                $success = true;
...
if ($success) {
    ...
    $redirect_to = root_dir() . 'add_plan.php?errormsg=true&' . $parametros;
    header('Location: ' . $redirect_to);
}
echo "fin";
//}