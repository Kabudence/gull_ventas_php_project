...
if (!empty($_POST)) {
    ...
    $product = [];
    $descuento_horario_image = $_FILES["horario_image"]["name"];
    $descuento_profesor_image = $_FILES["profesor_image"]["name"];

    $product['id'] = $_POST['descuento_id'];
...
//  echo print_r($product_inventory);
    ...
    if (empty($errors)) {
        ...
        if (empty($errors)) {
            ...
            if (!empty($product_image)) {
                ...
                $product_inserted_id = $_POST['descuento_id']; //insert_product($product);
                ...
                if ($product_inserted_id > -1) {
                    ...
                    foreach ($product_images as $image) {
                        ...
                        if (update_descuento_images($images) > -1) {
            ...
            if (!empty($descuento_horario_image)) {
            ...
            if (!empty($descuento_profesor_image)) {
    ...
    if ($success) {
        ...
        $redirect_to = root_dir() . 'add_descuento.php?descuento_id=' . $product['id'] . '&&errormsg=true';
        header('Location: ' . $redirect_to);
    }
}
?>