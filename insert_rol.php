...
if (!empty($_POST)) {
    ...
    $usuario['admin_id'] = $_POST['admin_id'];
    $rol['admin_id'] = $_POST['admin_id'];
...
//    }
    ...
    if (empty($errors)) {
        if (empty($errors)) {
            ...
            if (insertar_rol($rol) > -1) {                
                $success = true;
                $_SESSION['rol_msg'] = "Rol agregado con éxito.";
            }
        }
    }
    ...
    if ($success) {
        ...
        $redirect_to = root_dir() . 'add_rol.php?errormsg=true';
        header('Location: ' . $redirect_to);
    }
}
?>