...
if (!empty($_POST)) {
    ...
    $product = [];
    $plan_horario_image = $_FILES["horario_image"]["name"];
    $plan_profesor_image = $_FILES["profesor_image"]["name"];

    $product['id'] = $_POST['plan_id'];
    $product['title'] = $_POST['title'];
    $product['category'] = $_POST['category'];
    $product['description'] = $_POST['description'];
    $product['price'] = $_POST['price']; //price normal
    $product['price_plan'] = $_POST['price_plan']; //price plan
    ...
    $product['horario_image'] = $_POST['plan_img_horario'];
    $product['profesor_image'] = $_POST['plan_img_profesor'];
...
//  echo print_r($product_inventory);
        ...
        if (empty($errors)) {
            if (!empty($product_image)) {
                ...
                for ($i = 0; $i < count($_FILES['file']['name']); $i++) {
                    $target_path = "../public/uploads/planes/";
            ...
            if (!empty($product_image)) {
                ...
                $product_inserted_id = $_POST['plan_id']; //insert_product($product);
                ...
                if ($product_inserted_id > -1) {
                    ...
                    foreach ($product_images as $image) {
                        ...
                        $images['plan_id'] = $product_inserted_id;
                        if (update_plan_images($images) > -1) {
                            $_SESSION['product_msg'] = "Plan actualizado con éxito.";
                        }
                    }
                    $success = true;
                    $_SESSION['product_msg'] = "Plan actualizado con éxito.";
    ...
    if ($success) {
        ...
        $redirect_to = root_dir() . 'add_plan.php?plan_id=' . $product['id'] . '&&errormsg=true';
        header('Location: ' . $redirect_to);
    }
}
?>